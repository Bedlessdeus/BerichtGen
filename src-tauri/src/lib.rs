...
fn get_app_data_dir() -> Result<PathBuf, String> {
    ...
    if !berichtgen_dir.exists() {
        fs::create_dir_all(&berichtgen_dir)
...
async fn load_config() -> Result<Config, String> {
    ...
    if config_path.exists() {
        ...
        serde_json::from_str(&content).map_err(|e| format!("Could not parse config file: {}", e))
    } else {
        Ok(Config::default())
    }
}
...
async fn save_config(config: Config) -> Result<(), String> {
    ...
    fs::write(config_path, content).map_err(|e| format!("Could not write config file: {}", e))
}
...
async fn save_day_entry(
    ...
    if let Some(week_data) = data.get_mut(&week_key) {
        ...
        fs::write(week_file, content).map_err(|e| format!("Could not write week file: {}", e))?;
    }
...
async fn generate_pdf(year: i32, week: u32, state: State<'_, AppState>) -> Result<String, String> {
    ...
    let safe_trainee_name = config
        .trainee_name
        .replace(" ", "_")
        .replace("/", "_")
        .replace("\\", "_");
    let filename = format!(
        "Berichtsheft_KW{:02}_{}_{}_{}.txt",
        week,
        year,
        safe_trainee_name,
...
#[tauri::command]
async fn get_weeks_around_current(
    weeks_before: u32,
    weeks_after: u32,
...
#[tauri::command]
async fn get_self_directory() -> Result<String, String> {
