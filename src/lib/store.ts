...
export const currentWeek = writable<{ year: number; week: number }>({ year: 2025, week: 1 });
...
export const currentWeekData = derived(
  ...
  ([$currentWeek, $weekDataCache]) => {
    const key = `${$currentWeek.year}-${$currentWeek.week.toString().padStart(2, '0')}`;
...
export async function loadWeekData(year: number, week: number) {
  try {
    isLoading.set(true);
...
export async function saveDayEntry(year: number, week: number, weekday: string, entry: DayEntry) {
  try {
...
export async function generatePDF(year: number, week: number) {
  try {
    isLoading.set(true);
...
export async function getCurrentWeekInfo() {
  try {
    const [year, week] = await invoke<[number, number]>('get_current_week_info');
...
export async function getWeeksAroundCurrent(weeksBefore: number = 5, weeksAfter: number = 5) {
  try {
